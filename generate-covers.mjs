...
/* PostgREST treats commas, parens and dots as syntax inside a filter, so a
   value is double-quoted before it is percent-encoded. Storage paths with
   spaces or "v1.2" in them would otherwise silently match nothing. */
function eqFilter(v) {
  return "eq." + encodeURIComponent('"' + String(v).replace(/"/g, '\\"') + '"');
}

async function listDocuments() {
  const url =
    `${SUPABASE_URL}/rest/v1/documents` +
    `?select=title,storage_path,cover_path,type,published_on` +
...
async function saveCoverPath(storagePath, coverPath) {
  const url =
    `${SUPABASE_URL}/rest/v1/documents?storage_path=${eqFilter(storagePath)}`;
...
async function main() {
  ...
  for (const row of batch) {
    ...
    try {
      ...
      await saveCoverPath(row.storage_path, coverPath);
      done += 1;
